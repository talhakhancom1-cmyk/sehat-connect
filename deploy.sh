#!/bin/bash
# ================================================================
# Sehat Connect — Full VPS Deployment Script
...
REPO_URL="${REPO_URL:-git@github.com:talhakhancom1-cmyk/sehat-connect.git}"
BRANCH="${BRANCH:-main}"
...
if [ ! -f /opt/sehat-connect/.git ]; then
...
# Generate nginx.conf with the correct domain
cat > "$NGINX_SITE" <<NGINXEOF
# Sehat Connect — Nginx configuration for $DOMAIN
...
# Start with HTTP-only config first (so certbot can work)
# Temporarily comment out the HTTPS block
cp "$NGINX_SITE" "${NGINX_SITE}.backup"
sed -i 's/^server {/#server {/' "$NGINX_SITE"
sed -i 's/^    listen 443/#    listen 443/' "$NGINX_SITE"
sed -i 's/^    listen \[::\]:443/#    listen [::]:443/' "$NGINX_SITE"
...
# Restore full nginx config (with HTTPS block)
cp "${NGINX_SITE}.backup" "$NGINX_SITE"
rm -f "${NGINX_SITE}.backup"
...
echo "    pm2 logs sehat-connect-backend # View logs"
