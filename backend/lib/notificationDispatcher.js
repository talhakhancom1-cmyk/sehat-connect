...
async function deliverNotification(notification, io = null) {
  ...
  // 4. Email via SendGrid/SES (if configured)
  if (process.env.EMAIL_SERVICE && process.env.EMAIL_FROM) {
    try {
      const sent = await sendEmail(notification);
...
async function sendEmail(notification) {
  ...
  try {
    ...
    console.warn('SendGrid send error:', err.message);
    return false;
  }
}
