# EcoHealth — Developer Notes
**Production Server:** `ehcserver.webfrat.com`  
**Repository:** `github.com/talhakhancom1-cmyk/ecohealth`  
...
### Backend (`/opt/ecohealth/backend/.env`)
...
pm2 logs ecohealth-backend      # View logs (live)
pm2 logs ecohealth-backend --lines 50  # Last 50 lines
pm2 restart ecohealth-backend   # Restart backend
pm2 stop ecohealth-backend      # Stop backend
pm2 delete ecohealth-backend    # Delete from PM2
```
...
### PostgreSQL
```bash
sudo -u postgres psql -d ecohealth  # Connect to DB
...
mc ls local/ecohealth-uploads       # List files in bucket
```
...
```bash
# Backend logs
tail -f /opt/ecohealth/backend/logs/out.log
tail -f /opt/ecohealth/backend/logs/error.log

# Nginx logs
tail -f /var/log/nginx/ecohealth-access.log
tail -f /var/log/nginx/ecohealth-error.log
```
